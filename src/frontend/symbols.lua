...
function symbols:deps(ast)
	if traversal.unary[ast.type] then
		return self:collect(self:deps(ast.argument))
	elseif ast.type == "identifier" then
		return self:collect({
			ast,
		})
	elseif ast.type == "assignment" then
		return self:collect(self:deps(ast.name), self:deps(ast.value))
	elseif traversal.binary[ast.type] then
		return self:collect(self:deps(ast.left), self:deps(ast.right))
	elseif ast.type == "block" then
		return self:collect(self:deps(ast.body))
	elseif ast.type == "call" then
		return self:collect(self:deps(ast.name))
	elseif ast.type == "if" then
		return self:collect(self:deps(ast.condition), self:deps(ast.body), self:deps(ast.fallback))
	elseif ast.type == "while" then
		return self:collect(self:deps(ast.condition), self:deps(ast.body))
	elseif ast.type == "get" then
		return self:collect(self:deps(ast.argument), self:deps(ast.start), self:deps(ast.width))
	elseif ast.type == "set" then
		return self:collect(self:deps(ast.argument), self:deps(ast.start), self:deps(ast.width), self:deps(ast.value))
	else
		return
	end
end
...
function symbols:walk(ast, parent)
	...
	elseif ast.type == "assignment" then
		...
		self:add(ast.name, ast, self:deps(ast.value))
