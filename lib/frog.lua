...
function frog:print(...)
	if self.options["q"] or self.options["Q"] then
		return self
	end
	print(...)
	return self
...
function frog:throw(token, error, hint, type, a)
	...
	self:croak(
		self:colorize(type == "Warn" and colors.yellow or colors.red)
			.. (type or "Error")
	...
	if token and self.lines[token.position[1]] then
		...
		if self.lines[token.position[1] - 1] then
			...
			self:croak(
				self:colorize(colors.grey) .. "| " .. self:colorize(colors.reset) .. highlight(line, self.options)
			)
		end
		...
		self:croak(self:colorize(colors.grey) .. "| " .. self:colorize(colors.reset) .. highlight(line, self.options))
			:croak(
				self:colorize(colors.grey)
					...
					.. string.rep(
						"^",
						token.type == "string" and #token.characters + 2 or token.characters and #token.characters or 1
					)
					.. self:colorize(colors.reset)
			)
		self:croak(
